...
import { FileAction, fileActions } from "../FileActions"
...
const RunFileActionWindow: FunctionComponent<Props> = ({ actionName, filePaths, onClose, width, height }) => {
    ...
    const action: FileAction | undefined = useMemo(() => {
        return fileActions.find(a => (a.name === actionName))
    }, [actionName])
    ...
    const processorInfo: {appSpec: ComputeResourceSpecApp, app?: DendroComputeResourceApp, processor: ComputeResourceSpecProcessor} | undefined = useMemo(() => {
        ...
        for (const appSpec of computeResource?.spec?.apps || []) {
            ...
            for (const p of appSpec.processors || []) {
                if (p.name === action.processorName) {
                    return {appSpec, app, processor: p}
                }
            }
        }
        return undefined
    }, [computeResource, action])
    ...
    const handleSubmit = useCallback(async () => {
        ...
        for (let i = 0; i < filePaths.length; i++) {
            ...
            const outputFileName = `generated/${filePath2}/${action.outputFileBaseName}`
    ...
    if (!action) {
        return (
            <div>
                <span style={{color: 'red'}}>Unexpected: action not found: {actionName}</span>
                {cancelButton}
            </div>
        )
    }
    ...
    if (!processorInfo) {
        return (
            <div>
                <span style={{color: 'red'}}>Processor not found: {action.processorName}</span>
                {cancelButton}
            </div>
        )
    }
...
export default RunFileActionWindow